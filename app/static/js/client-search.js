...
function displayClientInfo(cliente) {
  ...
  if (nameElement) {
    const nombreCompleto = `${cliente.nombre_completo || ''} ${cliente.apellido_paterno || ''} ${cliente.apellido_materno || ''}`.trim();
  ...
  if (loanNotice && cliente.tiene_prestamo_activo) {
    ...
    if (loanDetailsText && cliente.prestamo_activo) {
      const fechaOtorgamiento = new Date(cliente.prestamo_activo.fecha_otorgamiento).toLocaleDateString('es-PE');
